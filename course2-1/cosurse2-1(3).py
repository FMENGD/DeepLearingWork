# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 23:15:13 2020
...
def forward_propagation(x,theta):
    """
    实现图中呈现的线性前向传播计算（计算J） J(theta) = theta * x
...
计算误差：
difference = ||grad - gradapprox|| / ||grad||2 + ||gradapprox||2  ...(4)
"""
...
def gradient_check(x,theta,epsilon=1e-7):
    ...
    numerator = np.linalg.norm(grad - gradapprox)                   #step1' 求分子 二范数
    denominator = np.linalg.norm(grad) + np.linalg.norm(gradapprox) #step2' 求分母
...
def backward_propagation_n(X,Y,cache):
    ...
    返回：
        gradients - 一个字典，其中包含与每个参数、激活、和激活前变量相关的成本梯度
